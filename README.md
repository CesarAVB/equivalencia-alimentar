...
API REST para gestão de equivalências nutricionais entre alimentos, com autenticação JWT,
controle de usuários por tipo e plano, e integração com Stripe para assinaturas recorrentes.
...
2. Configure o `application-local.properties` com suas credenciais:
   - **Banco de dados**: usuário e senha do MySQL
   - **JWT secret**: gere com `openssl rand -base64 64`
   - **Stripe**: chaves obtidas em https://dashboard.stripe.com/apikeys
...
br.com.sistema.alimentos
  ...
  ├── dtos
  │   ├── request     → Records de entrada (com validações Bean Validation)
  ...
  └── service         → Regras de negócio + JwtService + StripeService
```
...
Todas as rotas (exceto `/auth/login` e `/pagamentos/webhook`) exigem o header:

```
Authorization: Bearer <token>
```

O token é obtido via `POST /api/v1/auth/login` e expira em 24 h por padrão.

## Controle de acesso por papel

| Papel | Permissões |
|---|---|
| `ADMIN` | Acesso total: usuários, alimentos, equivalências |
| `NUTRICIONISTA` | Criar/editar alimentos e equivalências |
...
A documentação completa com exemplos de request/response está disponível no Swagger:
`http://localhost:8080/api/v1/swagger-ui.html`

## Integração Stripe

Antes de usar os endpoints de pagamento:
1. Configure os Price IDs no `StripeService.java` (constante `PRICE_IDS`)
2. Configure o webhook no Dashboard Stripe apontando para `/api/v1/pagamentos/webhook`
3. Adicione o `whsec_...` no `application-local.properties`
...
| `V2__seed_alimentos.sql` | Inserção dos 95 alimentos iniciais |
...
# Listar alimentos com filtro de grupo
curl "http://localhost:8080/api/v1/alimentos?grupo=Frutas&size=10" \
  -H "Authorization: Bearer <token>"
